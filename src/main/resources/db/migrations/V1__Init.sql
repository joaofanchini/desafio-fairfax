...
(
    ...
    confirm_check_in          boolean      not null,
    confirm_check_out         boolean      not null,
    paymentuuid               uuid,
    pre_authorize_paymentuuid uuid,
...
(
    ...
    max_guests integer,
    price      float(53),
    status     varchar(255) check (status in ('AVAILABLE', 'IN_USE', 'RESERVED')),
    primary key (id)
);
